...
public:
    ...
    virtual HRESULT WaitUntilConcurrentGCCompleteAsync(
        int millisecondsTimeout) override;
    ...
    virtual bool RegisterForFullGCNotification(uint32_t gen2Percentage,
        uint32_t lohPercentage) override;
    ...
    virtual int CollectionCount(int generation,
        int get_bgc_fgc_coutn = 0) override;

    virtual int StartNoGCRegion(uint64_t totalSize, bool lohSizeKnown,
        uint64_t lohSize, bool disallowFullBlockingGC) override;
    ...
    virtual HRESULT GarbageCollect(int generation = -1,
        bool low_memory_p = false, int mode = collection_blocking) override;
    ...
    virtual bool IsHeapPointer(void* object,
        bool small_heap_only = false) override;
    ...
    virtual bool IsThreadUsingAllocationContextHeap(gc_alloc_context* acontext,
        int thread_number) override;
    virtual bool IsEphemeral(Object* object) override;
    virtual uint32_t WaitUntilGCComplete(
        bool bConsiderGCStart = false) override;
    virtual void FixAllocContext(gc_alloc_context* acontext, void* arg,
        void* heap) override;
    ...
    virtual void SetYieldProcessorScalingFactor(
        float yieldProcessorScalingFactor) override;
    ...
    virtual Object* Alloc(gc_alloc_context* acontext, size_t size,
    ...
    virtual Object* GetContainingObject(void* pInteriorPtr,
        bool fCollectedGenOnly) override;
    virtual void DiagWalkObject(Object* obj, walk_fn fn,
        void* context) override;
    virtual void DiagWalkObject2(Object* obj, walk_fn2 fn,
        void* context) override;
    virtual void DiagWalkHeap(walk_fn fn, void* context, int gen_number,
        bool walk_large_object_heap_p) override;
    virtual void DiagWalkSurvivorsWithType(void* gc_context, record_surv_fn fn,
        void* diag_context, walk_surv_type type, int gen_number=-1) override;
    virtual void DiagWalkFinalizeQueue(void* gc_context,
        fq_walk_fn fn) override;
    virtual void DiagScanFinalizeQueue(fq_scan_fn fn,
        ScanContext* context) override;
    virtual void DiagScanHandles(handle_scan_fn fn, int gen_number,
        ScanContext* context) override;
    virtual void DiagScanDependentHandles(handle_scan_fn fn, int gen_number,
        ScanContext* context) override;
    ...
    virtual segment_handle RegisterFrozenSegment(
        segment_info *pseginfo) override;
    virtual void UnregisterFrozenSegment(segment_handle seg) override;
    virtual bool IsInFrozenSegment(Object* object) override;
    virtual void ControlEvents(GCEventKeyword keyword,
        GCEventLevel level) override;
    virtual void ControlPrivateEvents(GCEventKeyword keyword,
        GCEventLevel level) override;
    virtual unsigned int GetGenerationWithRange(Object* object,
    ...
    virtual void EnumerateConfigurationValues(void* context,
        ConfigurationValueFunc configurationValueFunc) override;
    virtual void UpdateFrozenSegment(segment_handle seg, uint8_t* allocated,
        uint8_t* committed) override;
    virtual int RefreshMemoryLimit() override;

    virtual enable_no_gc_region_callback_status EnableNoGCRegionCallback(
        NoGCRegionCallbackFinalizerWorkItem* callback,
        uint64_t callback_threshold) override;
